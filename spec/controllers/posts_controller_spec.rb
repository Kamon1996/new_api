  ...
  describe 'posts#create' do
    context 'with valid parameters' do
      it 'creates a new Post in DB' do
        expect do
          post :create, params: auth_headers.merge(post: new_post.as_json)
    ...
    context 'with invalid parameters' do
      it 'does not create a new Post' do
        expect do
          post :create, params: auth_headers.merge(post: new_post_invalid.as_json)
  ...
  describe 'posts#update' do
    context 'with valid parameters' do
      it 'updates the requested post' do
        expect do
          put :update, params: auth_headers.merge(id: created_post, post: new_post.as_json)
        end.to change(Post, :count).by(1)
        expect(json['title']).to eq(new_post['title'])
        expect(json['body']).to eq(new_post['body'])
        expect(json['id']).to eq(created_post['id'])
    ...
    context 'with invalid parameters' do
      it 'should send response with any errors' do
        put :update, params: auth_headers.merge(id: created_post, post: new_post_invalid.as_json)
        expect(response).to have_http_status(422)
